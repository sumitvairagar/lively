import 'package:lively/models/playlist.dart';
...
final livelyPlaylists = [
  LivelyPlayList("PL4cUxeGkcC9gjxLvV4VEkZ6H6H4yWuS58", "Material UI", null)
];

Future<void> loadAllData() async {
  var yt = YoutubeExplode();
  for (var lively in livelyPlaylists) {
    // Get playlist metadata.
    var playlist = await yt.playlists.get(lively.playListId);
    lively.playlist = playlist;


    Stream<Video> videos = yt.playlists.getVideos(playlist.id);

    await for (var video in videos) {
      lively.videos.add(video);
    }
  }
}
